...
const HomePage = () => {
  ...
  const fetchWatches = async () => {
    ...
    try {
      ...
      const { data } = await api.get(`/watches?${params.toString()}`);
  ...
  return (
    <div className="py-6">
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        ...
        <Link
          to="/add"
          className="btn btn-primary shadow-lg hover:shadow-primary/50 transition-shadow"
        >
          Add New Watch
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-base-200/50 backdrop-blur-sm rounded-2xl shadow-sm border border-base-300">
          <h2 className="text-2xl font-bold text-base-content/70 mb-3">
            No watches found
          </h2>

          <p className="text-base-content/50">
            Try adjusting your filters or add a new watch.
          </p>
        </div>
      ) : (
